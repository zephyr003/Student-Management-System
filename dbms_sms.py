...
connector = sqlite3.connect('StudentManagement.db', detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
...
def create_login_window():
    ...
    Label(login_window, text="Username:", font=("Helvetica", 14), bg='#3498DB', fg='white').grid(row=0, column=0, pady=10, padx=10)
    Label(login_window, text="Password:", font=("Helvetica", 14), bg='#3498DB', fg='white').grid(row=1, column=0, pady=10, padx=10)
    ...
    def validate_login():
        ...
        if username == "admin" and password == "admin123":
            ...
            main.deiconify()  # Show the main window after successful login
...
def add_record():
    ...
    if not name or not email or not contact or not gender or not DOB or not stream:
        ...
        mb.showerror('Error!', 'Please enter a valid 10-digit phone number.')
    else:
        try:
            connector.execute(
...
def reset_form():
    ...
    if mb.askyesno('Delete Database', 'Are you sure you want to delete the entire database?'):
        connector.execute('DROP TABLE IF EXISTS STUDENT_MANAGEMENT')
        connector.commit()
        mb.showinfo('Database Deleted', 'The entire database has been deleted successfully.')
        tree.delete(*tree.get_children())
        reset_fields()
...
Label(main, text="STUDENT MANAGEMENT SYSTEM", font=headlabelfont, bg='#3498DB', fg='white').pack(side=TOP, fill=X)
...
Label(left_frame, text="Contact Number", font=labelfont, bg=lf_bg, fg='white').place(relx=0.175, rely=0.18)
Label(left_frame, text="Email Address", font=labelfont, bg=lf_bg, fg='white').place(relx=0.2, rely=0.31)
Label(left_frame, text="Gender", font=labelfont, bg=lf_bg, fg='white').place(relx=0.3, rely=0.44)
Label(left_frame, text="Date of Birth (DOB)", font=labelfont, bg=lf_bg, fg='white').place(relx=0.1, rely=0.57)
Label(left_frame, text="Stream", font=labelfont, bg=lf_bg, fg='white').place(relx=0.3, rely=0.7)
...
Button(left_frame, text='Submit and Add Record', font=labelfont, command=add_record, width=18,
       bg=btn_bg, fg=btn_fg).place(relx=0.025, rely=0.85)

# Placing components in the center frame
Button(center_frame, text='Delete Record', font=labelfont, command=remove_record, width=15,
       bg=btn_bg, fg=btn_fg).place(relx=0.1, rely=0.25)

Button(center_frame, text='View Record', font=labelfont, command=view_record, width=15,
       bg=btn_bg, fg=btn_fg).place(relx=0.1, rely=0.35)

Button(center_frame, text='Reset Fields', font=labelfont, command=reset_fields, width=15,
       bg=btn_bg, fg=btn_fg).place(relx=0.1, rely=0.45)

Button(center_frame, text='Delete database', font=labelfont, command=reset_form, width=15,
       bg=btn_bg, fg=btn_fg).place(relx=0.1, rely=0.55)

# Placing components in the right frame
Label(right_frame, text='Students Records', font=headlabelfont, bg='#3498DB', fg='white').pack(side=TOP, fill=X)
